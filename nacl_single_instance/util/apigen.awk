...
function OutputPPBGlobalsHpp()
{
  ...
  for (i=1;i<=interface_count;i++)
  {
    ...
    print rpad( "extern const PPB_" interface_name "*", 40 )  rpad( "g_Nacl" interface_name ";", 30) "// (" function_count ") functions";
  }
  print "";
}
...
function OutputFunctionDeclarationHpp()
{
  ...
  {
    printf( rpad( "const char*", 30 ) "Nacl" interface_name function_name "( " );
  }
  else
  {
    printf( rpad( return_type, 30 ) "Nacl" interface_name function_name "( " );
  }
...
function OutputFunctionInlineHpp()
{
  ...
  printf( "Nacl" interface_name function_name "( " );
  ...
  printf( "g_Nacl" interface_name "->" function_name "(" );
...
function OutputAssignGlobals()
{
  ...
  for (i=1;i<=interface_count;i++)
  {
    ...
    interface_name = GetInterfaceName();
    function_count = GetFunctionCount();
    printf( rpad("g_Nacl" interface_name, 40) " = " rpad("(const PPB_" interface_name "*)",40) "get_browser_interface(PPB_" toupper(interface_name) ");\n");
  }
}
